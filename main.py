...
def is_tiktok_url(text):
    """Return True only for official TikTok URLs."""
    ...
    # TikTok links must use http/https. Add https for a pasted domain-only link.
    ...
    return host in official_hosts and bool(parsed.path)
...
class ShowcaseReviewView(
    ...
    async def approve(
        ...
        custom_text = s["showcase_message"] or "New TikTok submission from {user}!"
        custom_text = replace_placeholders(custom_text, member) if member else custom_text.replace("{user}", name)
...
class ShowcaseSubmitModal(discord.ui.Modal, title="Submit TikTok Video"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not is_tiktok_url(url):
            await interaction.response.send_message(
                "❌ Invalid TikTok link. Please paste an official TikTok link such as `https://vt.tiktok.com/...` or `https://www.tiktok.com/...`.",
                ephemeral=True
            )
            return
        ...
        if not s["showcase_enabled"]:
            await interaction.response.send_message("❌ The TikTok Showcase is currently turned off.", ephemeral=True)
            return

        review_channel = await get_channel(interaction.guild, s["showcase_review_channel_id"])
        if not review_channel:
            await interaction.response.send_message("❌ The rating channel is not configured.", ephemeral=True)
            return

        try:
            review_id = db_execute(
                """
                INSERT INTO showcase_reviews
                (guild_id, user_id, url, review_channel_id, status)
                VALUES (?, ?, ?, ?, 'pending')
                """,
                (interaction.guild.id, interaction.user.id, url, review_channel.id)
            )
            ...
            db_execute(
                "UPDATE showcase_reviews SET review_message_id = ? WHERE id = ?",
                (review_message.id, review_id)
            )

            await interaction.followup.send(
                "✅ Your TikTok video was sent to the rating/review channel for staff approval!",
                ephemeral=True
            )
        except Exception as error:
            print(f"Showcase submission error: {error}")
            if interaction.response.is_done():
                await interaction.followup.send(
                    "❌ Something went wrong while sending your submission. Please check that the bot can **View Channel, Send Messages, and Embed Links** in the rating/review channel.",
                    ephemeral=True
                )
            else:
                await interaction.response.send_message(
...
class ShowcaseSubmitView(discord.ui.View):
    ...
    async def submit_tiktok(self, interaction: discord.Interaction, button: discord.ui.Button):
...
async def showcase_panel(interaction):
    ...
    s = get_settings(interaction.guild.id)
    if not s["showcase_enabled"]:
        await interaction.response.send_message("❌ Turn on the Showcase first.", ephemeral=True)
        return

    if not s["showcase_review_channel_id"]:
        await interaction.response.send_message("❌ Configure the rating channel first.", ephemeral=True)
        return

    custom_message = s["showcase_message"] or "Click the button below to submit your TikTok video for review!"

    await interaction.response.send_message(
        embed=make_embed("🎬 TikTok Showcase", custom_message),
