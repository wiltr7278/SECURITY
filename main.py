...
def validate_assignable_role(guild, role):
    """Return an error message if a role cannot safely be used by SECURITY."""
    if role is None:
        return None

    if role.is_default():
        return "❌ You cannot use the @everyone role."
    ...
    if me is not None and role >= me.top_role:
        return "❌ I cannot manage that role. Move my bot role above it in Server Settings → Roles."
...
async def welcome_setup(
    ...
    if role is not None:
        role_error = validate_assignable_role(
            interaction.guild,
            role
        )
...
clean_group = app_commands.Group(
    name="clean",
    description="Clean server content"
)


def _clean_admin(interaction):
    return interaction.user.guild_permissions.administrator


@clean_group.command(name="channel", description="Delete a channel")
@app_commands.describe(channel="Channel to delete")
async def clean_channel(interaction, channel: discord.abc.GuildChannel):
    if not _clean_admin(interaction):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    name = channel.name
    try:
        await channel.delete(reason=f"SECURITY cleaner used by {interaction.user}")
        ...
        await interaction.followup.send("❌ I could not delete that channel.", ephemeral=True)


@clean_group.command(name="message", description="Delete messages from a channel")
@app_commands.describe(channel="Channel to clean", amount="Number of messages to delete (1-100)")
async def clean_message(
    ...
    try:
        ...
        await interaction.followup.send("❌ I do not have permission to manage messages there.", ephemeral=True)


@clean_group.command(name="role", description="Delete a role")
@app_commands.describe(role="Role to delete")
async def clean_role(interaction, role: discord.Role):
    if not _clean_admin(interaction):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
        return
    if role == interaction.guild.default_role:
        await interaction.response.send_message("❌ The @everyone role cannot be deleted.", ephemeral=True)
        return
    if role >= interaction.guild.me.top_role:
        await interaction.response.send_message("❌ I cannot delete a role that is equal to or higher than my top role.", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    name = role.name
    try:
        await role.delete(reason=f"SECURITY cleaner used by {interaction.user}")
        await interaction.followup.send(f"🧹 Deleted role **{name}**.", ephemeral=True)
    except discord.Forbidden:
        await interaction.followup.send("❌ I do not have permission to delete that role.", ephemeral=True)


@clean_group.command(name="category", description="Delete a category")
@app_commands.describe(category="Category to delete")
async def clean_category(interaction, category: discord.CategoryChannel):
    if not _clean_admin(interaction):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    name = category.name
    try:
        await category.delete(reason=f"SECURITY cleaner used by {interaction.user}")
        await interaction.followup.send(f"🧹 Deleted category **{name}**. Channels inside it were not deleted.", ephemeral=True)
    except discord.Forbidden:
        await interaction.followup.send("❌ I do not have permission to delete that category.", ephemeral=True)


@bot.tree.command(
    name="wipe",
    description="Delete all deletable channels and roles without deleting the server"
)
@app_commands.describe(confirm="Type WIPE to confirm")
async def wipe(interaction, confirm: str):
    if not _clean_admin(interaction):
        await interaction.response.send_message("❌ Administrator permission required.", ephemeral=True)
        return
    if confirm != "WIPE":
        await interaction.response.send_message(
            "❌ Confirmation failed. Type exactly `WIPE` in the confirm field.",
            ephemeral=True
        )
        return
    ...
    # Delete channels and categories first. The server itself is never deleted.
    for channel in list(guild.channels):
        try:
            await channel.delete(reason=f"SECURITY wipe used by {interaction.user}")
            deleted_channels += 1
        except (discord.Forbidden, discord.HTTPException):
            pass

    # Delete all roles the bot is allowed to manage, except @everyone.
    me = guild.me
    for role in list(guild.roles):
        ...
        if role.managed or role >= me.top_role:
            continue
        try:
            await role.delete(reason=f"SECURITY wipe used by {interaction.user}")
            deleted_roles += 1
        except (discord.Forbidden, discord.HTTPException):
            pass

    await interaction.followup.send(
        f"🧹 Wipe complete. Deleted **{deleted_channels}** channels/categories and **{deleted_roles}** roles. The server itself was not deleted.",
        ephemeral=True
    )


bot.tree.add_command(clean_group)
...
class ShowcaseSubmitModal(discord.ui.Modal, title="Submit TikTok Video"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        url = str(self.tiktok_url.value).strip()

        if not is_tiktok_url(url):
            await interaction.response.send_message(
                "❌ Please paste a valid TikTok URL.",
                ephemeral=True
            )
            return
        ...
        if not s["showcase_enabled"]:
            await interaction.response.send_message(
                "❌ The TikTok Showcase is currently turned off.",
                ephemeral=True
            )
            return
        ...
        if not review_channel:
            await interaction.response.send_message(
                "❌ The rating channel is missing. Ask an administrator to run `/showcase setup` again.",
                ephemeral=True
            )
            return
        ...
        cursor.execute(
            ...
            (guild_id, user_id, url, review_channel_id, status)
            VALUES (?, ?, ?, ?, 'pending')
            """,
            (
                interaction.guild.id,
        ...
        review_message = await review_channel.send(
            ...
            view=ShowcaseReviewView(review_id)
        )
        ...
        db_execute(
            ...
            WHERE id = ?
            """,
            (review_message.id, review_id)
        )

        await interaction.response.send_message(
            "✅ Your TikTok video was sent to the rating channel for review!",
            ephemeral=True
        )


class ShowcaseSubmitView(discord.ui.View):

    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(
        label="Submit TikTok Video",
        emoji="🎬",
        style=discord.ButtonStyle.primary,
    ...
    async def submit_tiktok(
        ...
        await interaction.response.send_modal(ShowcaseSubmitModal())
...
@app_commands.describe(
    custom_message="Text shown on the showcase panel",
    rating_channel="Staff channel for approving or rejecting submissions",
    enabled="Turn the TikTok Showcase on or off"
)
async def showcase_setup(
    interaction: discord.Interaction,
    custom_message: str,
    rating_channel: discord.TextChannel,
    enabled: bool
):
    ...
    update_setting(gid, "showcase_message", custom_message)
    update_setting(gid, "showcase_review_channel_id", rating_channel.id)
    update_setting(gid, "showcase_enabled", 1 if enabled else 0)

    status = "ON" if enabled else "OFF"

    await interaction.response.send_message(
        f"✅ TikTok Showcase setup saved.\n"
        f"📝 **Custom message:** {custom_message}\n"
        f"⭐ **Rating channel:** {rating_channel.mention}\n"
        f"🔘 **Status:** {status}\n\n"
        f"Use `/showcase panel` in the channel where you want the Showcase panel to appear.",
        ephemeral=True
    )
...
async def showcase_panel(interaction: discord.Interaction):
    ...
    if not s["showcase_review_channel_id"]:
        await interaction.response.send_message(
            "❌ Configure the rating channel first with `/showcase setup`.",
            ephemeral=True
        )
        return

    # The channel where /showcase panel is used becomes the public showcase channel.
    update_setting(
        interaction.guild.id,
        "showcase_channel_id",
        interaction.channel.id
    ...
    panel_message = (
        s["showcase_message"]
        or "Click the button below to submit your TikTok video for review!"
    )
    ...
    await interaction.response.send_message(
        embed=make_embed(
            "🎬 TikTok Showcase",
            f"{panel_message}\n\n"
            "Click the button below to submit your TikTok video."
        ),
        view=ShowcaseSubmitView()
    )
...
async def handle_showcase_message(
    ...
    # Showcase submissions are handled through the /showcase panel button and modal.
    # This keeps normal TikTok links from being submitted twice.
    return
...
async def help_command(interaction):
    ...
    e.add_field(
        ...
        value=(
            ...
            "Automatic anti-link, anti-spam and mention protection"
        ),
        inline=False
    )
    ...
    e.add_field(
        name="🧹 Cleaner",
        value=(
            "`/clean channel` `/clean message`\n"
            "`/clean role` `/clean category`\n"
            "`/wipe confirm:WIPE`"
        ),
        inline=False
    )
